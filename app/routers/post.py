...
def get_posts(db:Session = Depends(get_db),current_user : int = Depends(oauth2.current_user),limit: int=7, 
...
def create_post(post : schemas.PostCreate,db:Session = Depends(get_db), current_user : int = Depends(oauth2.current_user)):
...
def update_post(id:int, updated_post:schemas.PostCreate,db:Session = Depends(get_db),current_user : int = Depends(oauth2.current_user)):
